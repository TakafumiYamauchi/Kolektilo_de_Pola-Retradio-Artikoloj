...
## 3) 長期間を高速に取得する（並列 CLI）

- `parallel_scraper.py` は指定期間を自動でほぼ等分し、`--workers` で指定した本数のプロセスを並列実行します。2011年以降すべてを 16～32 コアで一気に取得したいケースに向いています。
- 実行例（16 分割 + 年別出力）:
  ```bash
  python parallel_scraper.py \
      --start 2011-01-01 --end 2025-10-20 \
      --workers 16 --split-by year --out output_parallel --throttle 0.7
  ```
  - `--workers` を 32 にすれば 32 区間まで分割できます（期間が短い場合は自動調整）。
  - `--throttle` は並列数に応じて必ず見直してください。並列実行時もサイトへの負荷を避けるため **過度なリクエスト頻度にならない** よう調整しましょう。
- ワーカー数が 2 以上の場合は SQLite ベースの `requests-cache` を自動で無効化します。並列前に単独で実行済みなら、既存キャッシュは現状維持です。
- HPC 環境で多数コアを使う場合は、`reference.sh` のジョブスクリプトを流用して `python parallel_scraper.py ...` を呼び出してください。`-pe` のコア数や `OMP_NUM_THREADS` の値を 16/32 などに調整すれば、そのまま並列取得が動作します。
